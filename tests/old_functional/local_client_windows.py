...
class WindowsLocalClient(LocalTest):
    ...
    def do_op(self, op: int, path_from: Path, path_to: Path, flags: int = 0) -> None:
    ...
    def delete(self, ref: RawPath) -> None:
        ...
        self.do_op(
            ...
            self.abspath(ref),
            None,
            flags=shellcon.FOF_NOCONFIRMATION | shellcon.FOF_ALLOWUNDO,
        )
    ...
    def move(self, ref: RawPath, new_parent_ref: RawPath, name: str = None) -> None:
        ...
        self.do_op(shellcon.FO_MOVE, path, new_path, flags=shellcon.FOF_NOCONFIRMATION)

    def rename(self, srcref: RawPath, to_name: str) -> Path:
        path = self.abspath(srcref)
        new_path = path.with_name(to_name)
        self.do_op(
            shellcon.FO_RENAME, path, new_path, flags=shellcon.FOF_NOCONFIRMATION
        )
        time.sleep(0.5)
        return new_path
